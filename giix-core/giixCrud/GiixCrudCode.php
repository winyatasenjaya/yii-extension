...
class GiixCrudCode extends CrudCode {
	 ...
	 * @param string $htmlClassAttr name of html class attribute
	 * @return string The source code line for the active field.
	 * @author Rodrigo Coelho <rodrigo@giix.org>
	 * @author Winyata Senjaya <w.senjaya@gmail.com>
	 */
	public function generateActiveField($modelClass, $column, $htmlClassAttr='') {

		$classAttr = ($column->type==='string') ? ", 'class'=>'".$htmlClassAttr."'" : ", array('class'=>'".$htmlClassAttr."')";

		if ($column->isForeignKey) {
			$relation = $this->findRelation($modelClass, $column);
			$relatedModelClass = $relation[3];
			return "echo \$form->dropDownList(\$model, '{$column->name}', GxHtml::listDataEx({$relatedModelClass}::model()->findAllAttributes(null, true))".$classAttr.")";
		}
		...
		if (strtoupper($column->dbType) == 'TINYINT(1)'
			...
			return "echo \$form->textArea(\$model, '{$column->name}'".$classAttr.")";
		} else {
			...
			if ($column->type !== 'string' || $column->size === null)
				return "echo \$form->{$inputField}(\$model, '{$column->name}'".$classAttr.")";
			else
				return "echo \$form->{$inputField}(\$model, '{$column->name}', array('maxlength' => {$column->size}".$classAttr."))";
	 ...
	 * @return string The source code line for the column definition.
	 */
	public function generateSearchField($modelClass, $column, $htmlClassAttr='') {
		if (!$column->isForeignKey) {
			// Boolean or bit.
			if (strtoupper($column->dbType) == 'TINYINT(1)'
					|| strtoupper($column->dbType) == 'BIT'
				...
				return $this->generateActiveField($this->modelClass, $column, 'form-control');
		} else { // FK.
			// Find the related model for this column.
			$relation = $this->findRelation($modelClass, $column);
			$relatedModelClass = $relation[3];
...
EOM;
	...
	/**
	 * Generates and returns the view source code line
	 * to create the active label based on
	 * the model attribute field type on the database.
	 * #MethodTracker
	 * This method overrides {@link CrudCode::generateActiveLabel}, from yii Core generator crud
	 * @param string $modelClass The model class name.
	 * @param CDbColumnSchema $column The column.
	 * @param string $htmlClassAttr name of html class attribute
	 * @return string The source code line for the active label.
	 * @author Winyata Senjaya <w.senjaya@gmail.com>
	 */
	public function generateActiveLabel($modelClass,$column, $htmlClassAttr='')
	{
		return "\$form->labelEx(\$model,'{$column->name}', array('class'=>'".$htmlClassAttr."'))";
	}
